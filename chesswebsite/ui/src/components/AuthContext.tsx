...
export function AuthProvider({ children }: PropsWithChildren){
  ...
  const csrftoken = useMemo(()=>(document.cookie.split('; ').find(row => row.startsWith('csrftoken=')) || '').split('=')[1], []);
  ...
  async function register(payload: { username: string; email?: string; password: string; password2?: string }){
